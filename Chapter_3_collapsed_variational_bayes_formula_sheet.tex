...
&\quad \quad \quad \quad \tau_{\sigma^2} \cancel{\left( 1 + \tau_g \right)} \cancel{\left( 1 + \tau_g \right)^{-1}} \tr(\cancel{\mX^\top \mX} \cancel{(\mX^\top \mX)^{-1}}) - 2 (1 + \tau_g)^{-1} \vy^\top \mX (\mX^\top \mX)^{-1} \mX^\top \vy ] \Bigg \} \\
&= \exp \left \{ -\left(\frac{n + p}{2} + 1 \right)\log \sigma^2 - \frac{1}{2 \sigma^2} \{n[1 - (1 + \tau_g)^{-1}  R^2] + \tau_{\sigma^2} p \} \right \}
\end{align*}

and thus $q^*(\sigma^2) \sim \IG(\alpha_{q(\sigma^2)}, \beta_{q(\sigma^2)})$ where
$\alpha_{q(\sigma^2)} = \frac{n + p}{2}$ and $\beta_{q(\sigma^2)} = \frac{1}{2} \{n[1 - (1 + \tau_g)^{-1}  R^2] + \tau_{\sigma^2} p \}$.

\subsection{$q(g)$}
\begin{align*}
q(g) &\propto \exp \E_{-g} \left\{-\frac{p}{2} \log g - \frac{1}{g} \frac{1}{2 \sigma^2} \vbeta^\top \mX^\top \mX \vbeta + b \log g - (a + b + 2) \log(1 + g) \right\} \\
&= \exp \left \{ -\frac{p}{2} \log g - \frac{1}{2} \frac{1}{g} \E \left[ \frac{1}{\sigma^2} \right] \E (\mX \vbeta)^\top (\mX \vbeta) + b \log g - (a + b + 2) \log(1 + g) ] \right \} \\
&= \exp \left \{ -\frac{p}{2} \log g - \frac{1}{2} \frac{1}{g} \tau_{\sigma^2} \tr[(\mSigma_{q(\vbeta)} + \vmu_{q(\vbeta)} \vmu_{q(\vbeta)}^T) \mX^\top \mX] + b \log g - (a + b + 2) \log(1 + g) ] \right \} \\
&= \exp \left \{ \left(b - \frac{p}{2}\right) \log g - \frac{1}{g} \frac{1}{2} \left[\tau_{\sigma^2} (1 + \tau_g)^{-1} \vy^\top \mX^\top \cancel{(\mX^\top \mX)^{-1}} \cancel{\mX^\top \mX} (\mX^\top \mX)^{-1} \mX \vy + \tr{[\tau_{\sigma^2}^{-1} (1 + \tau_g)^{-1} \cancel{\mX^\top \mX} \cancel{(\mX^\top \mX)^{-1}}]} \right] \right \} \\
& \quad \quad \qquad	- (a + b + 2) \log(1 + g)  \biggl \} \\
