...
\begin{align*}
	...
	&= \E_g\left[\frac{g}{1 + g} \E[\sigma^2|\vy, g] (\mX^\top \mX)^{-1} | \vy \right] + \Var_g\left[\frac{g}{1 + g} \vbetahatls \vbetahatls^\top\right] \\
	&= \frac{n}{n - 2} \E_g\left[\frac{g}{1 + g} \left(1 - \frac{g}{1 + g} R^2 \right) \Bigm| \vy \right] (\mX^\top \mX)^{-1} + \Var_g\left[\frac{g}{1 + g} \right] \vbetahatls \vbetahatls^\top \\
\end{align*}

\begin{align*}
	\E\left(\frac{g}{1 + g} | \vy \right) &= \int_0^\infty \frac{g}{1 + g} p(g | \vy) dg \\
	&= \frac{(1 - R^2)^{b+1}}{\Beta(p/2 + a + 1, b + 1)} \int_0^\infty g^{b+1} (1 + g)^{-1} [1 + g (1 - R^2)]^{-n/2} dg\\
	&= \frac{\Beta(p/2 + a + 1, b + 2)}{\Beta(p /2 + a + 1, b + 1)} (1 - R^2)^{b+1} {}_2 F_1(p/2 + a + 1, 1; n/2 + 1; R^2)
\end{align*}
...
when $[|\arg \alpha| < \pi], -\Re(\mu + \mu) > \Re \lambda > 0$

\[
	\E\left[ \left(\frac{g}{1 + g} \right)^2 | \vy \right] = \frac{\Beta(p/2 + a + 1, b + 2)}{\Beta(p /2 + a + 1, b + 1)} (1 - R^2)^{b+2} {}_2 F_1(p/2 + a + 1, 1; n/2 + 1; R^2)
\]
