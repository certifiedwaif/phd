...
# Outline: Need to generate spline data

fit_spline <- function(x, y) {
    # x could be anything you like y could be anything you like
    a <- min(x)
    b <- max(x)
    # x <- radiation y <- ozone^(1/3) a <- 0 b <- 350
...
ZOSull <- function(x, range.x, intKnots, drv = 0, stability_check = FALSE) {
    ...
    if (missing(intKnots)) {
        ...
        intKnots <- quantile(unique(x), seq(0, 1, length = (numIntKnots + 2))[-c(1, 
            (numIntKnots + 2))])
    }
...
formOmega <- function(a, b, intKnots) {
    allKnots <- c(rep(1.05 * a, 2), intKnots, rep(1.05 * b, 4))
    u <- unique(allKnots)
    R <- length(u)
    xtilde <- c()
    wts <- c()
...
ZOSull2 <- function(x, range.x, intKnots, allKnots = NULL, Omega = NULL, drv = 0, 
