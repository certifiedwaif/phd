...
res.glm <- glm(vy~-1+mX,family=binomial)
...
djointLMM <- function(u,v,vy,mX,mSigma.inv) {
		...
		for (i in 1:length(u)) {
		    for (j in 1:length(v)) {
		        ...
		        veta <- mX%*%vtheta
		        log.f[i,j] <- sum(vy*veta - log(1+exp(veta))) - 0.5*t(vtheta)%*%mSigma.inv%*%vtheta
		    }
		}
...
fast.f <- function(mTheta,vy,mX,mSigma) 
{
    ...
    log.vp <- matrix(1,1,n)%*%(mY*mEta - log(1 + exp(mEta))) + dmvnorm(mTheta,sigma=mSigma,log=TRUE)
    return(log.vp)
} 
