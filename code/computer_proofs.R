...
for (i in 1:1000) {
    ...
    integrand <- exp(-(n * alpha^2)/(2 * sigma2) + -(c + 3/2) * log(sigma2) - n/(2 * 
        sigma2)) * hyperg_1F1(b + 1, c, (n * R2)/(2 * sigma2))
    
    numeric <- exp(log_K) * trapint(sigma2, integrand)
    
    analytic <- exp(lgamma(c + 0.5) + (b + 1) * log(1 - R2) - lgamma(c) - 0.5 * log(pi) - 
        n/2 * log(1 + alpha^2)) * hyperg_2F1(b + 1, c + 0.5, c, R2/(1 + alpha^2))
...
for (i in 1:1000) {
    ...
    analytic <- exp(log_K + lbeta(p/2 + a + 1, b + 3)) * hyperg_2F1(n/2, b + 3, n/2 + 
        2, R2)
