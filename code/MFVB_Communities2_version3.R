...
plot(NA, type = "n", xlim = c(0, p + 1), ylim = c(0, p + 1), xlab = "variable index", 
    ylab = "variable index", main = "correlation - complete clustering", cex.lab = 1.5, 
    cex.main = 2)
for (j in 1:p) {
    for (k in 1:p) {
        col = "white"
...
for (ITER in 1:MAXITER) {
    ...
    if (nFixed == 0) {
        ...
        res[1] <- 0
        # b0 <- proc.time()[3] t0 <- b0-a0 cat(t0,'\n')
        ...
        # a0 <- proc.time()[3]
        res <- all_correlations_mX_mZ(vy, mX[, fixedInds], matrix(mX[, inds], n, 
            length(inds)), intercept_col = 0, bIntercept = FALSE, bCentre = FALSE, 
            cores = 1)
        # res[1] <- 0 b0 <- proc.time()[3] t0 <- b0-a0 cat(t0,'\n')
        
        if (FALSE) {
            
            a0 <- proc.time()[3]
            res2 <- my.all_correlations_mX_mZ(vy, mX[, fixedInds], matrix(mX[, inds], 
                n, length(inds)))
            b0 <- proc.time()[3]
            t0 <- b0 - a0
            cat(t0, "\n")
