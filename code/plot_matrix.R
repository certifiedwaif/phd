...
plot_matrix <- function(p, q, m, chol = FALSE) {
    ...
    idx <- p + 1
    for (i in 1:q) {
        # inds <- (p + b * (m-1) + 1):((p + b * m)) cat('idx', idx, '\n')
        for (j in 1:m) {
            for (k in 1:m) {
...
x <- c(0, 7, 3, 4, 5, 3, 2, 6, 5, 0, 0, 1, 0, 0, 5, 0, 2, 3, 6, 4, 0, 5, 4, 0, 7, 
    0, 0, 0, 7, 0, 6, 6, 0, 3, 0, 5, 0, 4, 0, 0, 0, 2, 3, 0, 3, 4, 5, 0, 8, 0)
pdf("~/Dropbox/phd/poisson_does_not_fit.pdf", width = 3, height = 3)
hist(x, , prob = TRUE)
lambda_hat <- mean(x)
points(0:8 + 0.5, dpois(0:8, lambda_hat), col = "red", cex = 1.25, pch = 19)
dev.off()
