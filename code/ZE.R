...
CalcSelectionScores <- function(vgamma, vgamma.hat) {
    ...
    return(list(TP = TP, TN = TN, FP = FP, FN = FN, sensitivity = sensitivity, specificity = specificity, 
        precision = precision, recall = recall, accuracy = accuracy, F1 = F1, BER = BER))
}
...
ZE.exact.fast <- function(vy, mX, LARGEP) {
    ...
    for (i in 1:p) {
        lmZ[[i]] <- matrix(0, i, i)
        inds <- 1:(i - 1)
        mZ11 <- matrix(FALSE, i, i)
        mZ11[inds, inds] <- TRUE
        mZ12 <- matrix(FALSE, i, i)
        mZ12[inds, i] <- TRUE
        mZ21 <- matrix(FALSE, i, i)
        mZ21[i, inds] <- TRUE
        mZ22 <- matrix(FALSE, i, i)
        mZ22[i, i] <- TRUE
    ...
    for (j in 2:nrow(mA)) {
        # cat('Iteration', j, '\n')
        if (vs[j - 1]) {
            # Adding variable cat('Adding', vw[j-1], '\n')
            indsNew <- c(inds, vw[j - 1])
            # cat('indsNew', indsNew, '\n')
        } else {
            # Removing varable cat('Removing', vw[j-1], '\n')
            k <- which(inds == vw[j - 1])
            indsNew <- inds[-k]
            # cat('indsNew', indsNew, '\n')
        }
