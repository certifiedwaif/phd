...
if (FALSE) {
    ...
    b3 <- proc.time()[3]
    print(b3 - a3)
    write.table(res.exact1$vR2, sep = ",", file = "Hitters_exact1.csv", row.names = FALSE, 
        col.names = FALSE)
}
...
if (TRUE) {
    ...
    # shine(l1)
    write.table(res.exact2$vR2, sep = ",", file = "Hitters_exact2.csv", row.names = FALSE, 
        col.names = FALSE)
}
...
# shine(l2) max(abs(res.exact3$vlog.ZE - res.exact2$vlog.ZE))
