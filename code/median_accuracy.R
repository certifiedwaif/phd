...
generate_test_case <- function(i, test) {
    ...
    if (test == "intercept") {
        ...
        result <- generate_slope_test_data(m = 20, ni = 10, expected_beta = c(2, 
            1), expected_rho = 0.7)
    } else if (test == "spline") {
        # result <- generate_spline_test_data() mult <- result$mult allKnots <-
        # result$allKnots
...
save_mcmc <- function(test) {
    ...
    accuracy <- mclapply(1:ITER, function(i) {
        # accuracy <- lapply(97, function(i) { Idea: Save generated data so we can
        # reproduce problem data sets?
        result <- generate_test_case(i, test)
        if (test == "spline") {
            mult <- result$mult
            stan_fit <- mcmc(mult, seed = i, iterations = 1e+05, warmup = 1000, stan_file = "multivariate_zip_splines.stan")
        } else {
            mult <- result
            stan_fit <- mcmc(mult, iterations = 1e+05, warmup = 10000, mc.cores = 1)
        }
        hostname <- Sys.info()["nodename"]
        if (hostname == "verona.maths.usyd.edu.au") 
            save(stan_fit, file = sprintf("/dskh/nobackup/markg/phd/stan_fit_%s_%d.RData", 
                test, i)) else if (hostname == "markg-OptiPlex-9020") 
            save(stan_fit, file = sprintf("/home/markg/phd_data/stan_fit_%s_%d.RData", 
                test, i)) else stop("Cannot find hostname")
    }, mc.cores = 40)
}
...
load_stan_data <- function(i, test) {
    hostname <- Sys.info()["nodename"]
    if (hostname == "verona.maths.usyd.edu.au") 
        load(file = sprintf("/dskh/nobackup/markg/phd/stan_fit_%s_%d.RData", test, 
            i)) else if (hostname == "markg-OptiPlex-9020") 
        load(file = sprintf("/home/markg/phd_data/stan_fit_%s_%d.RData", test, i)) else stop("Cannot find hostname")
    return(stan_fit)
}
...
median_accuracy <- function(approximation = "gva", test = "intercept", save_flag = FALSE) {
    if (save_flag) {
        ...
        accuracy <- lapply(1:ITER, function(i) {
            ...
            # If the integration fails, something must have gone fundamentally wrong, and we
            # should figure out why.
            if (test == "spline") {
                vy <- mult$vy
                vx <- mult$mX[, 1]
                xtilde <- seq(from = -1, to = 1, by = 0.01)
                result <- spline.des(allKnots, xtilde, derivs = rep(0, length(xtilde)), 
                  outer.ok = TRUE)
                ...
                result <- calculate_accuracies("", mult, mcmc_samples, var_result, 
                  approximation, print_flag = FALSE, plot_flag = TRUE)
            }
            # In spline case, should calculate accuracy of the fit functions
            save(result, file = sprintf("results/accuracy_result_%s_%s_%d.RData", 
                approximation, test, i))
            result
        })  # , mc.cores = 32)
        
        save(accuracy, file = sprintf("results/accuracy_list_%s_%s.RData", approximation, 
            test))
    }
...
create_accuracy_df <- function(accuracy, make_colnames = FALSE) {
    ...
    if (make_colnames) {
        colnames(accuracy_df) <- c("vbeta_0", "vbeta_1", sapply(1:length(l$vu_accuracy), 
            function(x) paste0("vu_", x)), sapply(1:length(l$sigma2_vu_accuracy), 
            function(x) paste0("sigma2_vu_", x)), "rho")
    }
    return(accuracy_df)
}

# Somehow do this across all approximations.  So for each parameter, plot the
# Laplace, GVA, GVA2 and GVA2 accuracy But label the x axis only for the
# parameter. Use a legend to indicate the approximation
median_accuracy_graph <- function(approximation = "gva", test = "intercept") {
    accuracy_df <- data.frame()
    # Create accuracy data frame Merge with other accuracy data frames
    ...
    # Create graph Order by parameter, then approximation within that
    ...
    boxplot(as.numeric(as.character(vbeta_0)) ~ approximation, main = "vbeta_0", 
        data = accuracy_df)
    boxplot(as.numeric(as.character(vbeta_1)) ~ approximation, main = "vbeta_1", 
        data = accuracy_df)
    boxplot(as.numeric(as.character(sigma2_vu_1)) ~ approximation, main = "sigma2_vu_1", 
        data = accuracy_df)
    boxplot(as.numeric(as.character(sigma2_vu_2)) ~ approximation, main = "sigma2_vu_2", 
        data = accuracy_df)
    par(mfrow = c(1, 1))
    if (test == "intercept") {
        mean_vu <- apply(accuracy_df[, 3:22], 1, mean)
        accuracy_df2 <- cbind(accuracy_df[, c("vbeta_0", "vbeta_1", "sigma2_vu", 
            "rho")])
        boxplot(accuracy_df2, ylim = c(0, 1))
        axis(1, at = 1:5, labels = c(expression(bold(beta)[0], bold(beta)[1], bold(u)[1], 
            bold(sigma[u]^2), rho)))
    } else if (test == "slope") {
        ...
        mean_vu_df <- data.frame(mean_vu_0 = mean_vu[, 1], mean_vu_1 = mean_vu[, 
            2])
        accuracy_df2 <- cbind(accuracy_df[, c("vbeta_0", "vbeta_1")], mean_vu_df, 
            accuracy_df[, c("sigma2_vu_1", "sigma2_vu_2", "rho")])
        colnames(accuracy_df2) <- rep("", 7)
        boxplot(accuracy_df2)  # , ylim=c(0, 1))
        axis(1, at = 1:7, labels = c(expression(bold(beta)[0], bold(beta)[1], bold(u)[1], 
            bold(u)[2], bold(sigma[u1]^2), bold(sigma[u2]^2), rho)))
    } else if (test == "spline") {
...
median_accuracy_graph_all <- function(test) {
    # for (approximation in c('gva', 'gva2')) { for (approximation in c('laplace',
    # 'gva', 'gva2', 'gva_nr')) { median_accuracy_graph(approximation=approximation,
    # test=test) median_accuracy_csv(approximation=approximation, test=test) }
    ...
    # Construct a data frame with a column for each combination of approximation and
    # parameter
    acc <- data.frame(a = rep(NA, 100))
    for (param in parameters) {
        for (approximation in c("laplace", "gva", "gva2", "gva_nr")) {
            # Take the mean of the vu accuracies
            accuracy_numbers <- as.numeric(as.character(accuracy_df2[accuracy_df$approximation == 
                approximation, param]))
            acc <- cbind(acc, accuracy_numbers)
            colnames(acc)[length(colnames(acc))] <- sprintf("%s_%s", approximation, 
                param)
        }
    }
    acc <- acc[, 2:ncol(acc)]
    ...
    # if (test == 'slope') { acc <- 100.0 * acc }
    boxplot(acc, col = gray((1:4)/(2:5)), xaxt = "n", ylim = c(0, 100), ylab = "Accuracy")
    if (test == "intercept") {
        axis(1, labels = c(expression(bold(beta)[0]), expression(bold(beta)[1]), 
            expression(bold(u)[0]), expression(sigma[bold(u)[0]]), expression(rho)), 
            at = 1:5 * 4 - 0.25, tick = TRUE)
    } else if (test == "slope") {
        axis(1, labels = c(expression(bold(beta)[0]), expression(bold(beta)[1]), 
            expression(bold(u)[0]), expression(bold(u)[1]), expression(sigma[bold(u)[0]]), 
            expression(sigma[bold(u)[1]]), expression(rho)), at = 1:7 * 4 - 0.25, 
            tick = TRUE)
    }
    legend("bottomright", c("Laplace", latex2exp("GVA $\\Lambda = R^T R$"), latex2exp("GVA $\\Lambda = (R^T R)^{-1}$"), 
        "GVA fixed point"), lty = 1, col = gray((1:4)/(2:5)))
    title(sprintf("Combined median accuracy graph - %s", test))
    dev.off()
    # TODO: Label the axes better TODO: Why are the accuracies for the variance
    # components so low, although the estimation for the vbeta components is better?
}
...
# Graph of Var_q(theta) against Var(theta|y) How to get this?  Run fits for a
...
coverage_percentage <- function(approximation = "gva", test = "intercept") {
    # Percentage coverage of the true parameter values by approximate 95% credible
    # intervals based on variational Bayes approximate posterior density functions.
    # The percentages are based on 10,000 replications.
    ...
    expected_beta <- c(2, 1)
    expected_rho <- 0.7
    result <- matrix(0, 10000, 42)
    for (i in 1:10000) {
        set.seed(i)
        cat("i", i, "counter", counter, "percentage", round(100 * counter/i, 2), 
            "\n")
        # mult <- generate_test_data(m, ni, expected_beta = expected_beta, expected_rho =
        # expected_rho)
        mult <- generate_test_case(i, test)
        
        # var_result <- tryCatch(zipvb(mult, method=approximation, verbose=FALSE), error
        ...
        if (is.between(0, sum_vmu_vu - 1.96 * sum_mLambda_vu, sum_vmu_vu + 1.96 * 
            sum_mLambda_vu)) {
...
mean_var <- function(vbeta) {
    ...
    return(with(result, {
        ...
        list(var_approx_mean = var_approx_mean, mcmc_approx_mean = mcmc_approx_mean, 
            var_approx_var = var_approx_var, mcmc_approx_var = mcmc_approx_var)
    }))
}

# This is most definitely a verona job
print_mean_var <- function() {
    for (theta in seq(1, 2, by = 0.01)) print(mean_var(c(1, theta)))
}
...
main <- function() {
    option_list <- list(make_option(c("-a", "--approximation"), default = "gva"), 
        make_option(c("-t", "--test"), default = "intercept"), make_option(c("-c", 
            "--coverage_percentage"), action = "store_true", default = FALSE), make_option(c("-m", 
            "--median_accuracy"), action = "store_true", default = FALSE), make_option(c("-s", 
            "--save_flag"), action = "store_true", default = FALSE), make_option(c("-d", 
            "--save_mcmc"), action = "store_true", default = FALSE))
