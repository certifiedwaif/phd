...
posterior.sigma2GivenY.exact <- function(sigma2, n, p, R2) {
    ...
    log.f = log.f + log(hyperg_1F1(d + 1, c, -0.5 * n * R2/sigma2, give = FALSE, 
        strict = TRUE))
...
posterior.alphaGivenY.exact <- function(alpha, n, p, R2) {
    ...
    log.f1 = log.f1 + log(hyperg_2F1(b + 1, c + 0.5, c, R2/(1 + alpha^2), give = FALSE, 
        strict = TRUE))
    ...
    log.f2 = log.f2 + log(hyperg_2F1(d + 1, -0.5, c, R2/(1 + alpha^2), give = FALSE, 
        strict = TRUE))
