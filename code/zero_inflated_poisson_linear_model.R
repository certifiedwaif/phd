# FIXME: Extend this code to the zero-inflated case
...
f.lap.zero <- function(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv) 
{       
    vtheta <- c(vbeta, vu)
    d <- length(vtheta)
    veta <- cbind(mX, mZ)%*%vtheta
    f <- sum(vr*vy*veta - vr*exp(veta)) - 0.5*(t(vbeta)%*%mSigmaBeta.inv%*%vbeta + t(vu)%*%mSigma.inv%*%vu)
    return(f)
}

###############################################################################

vg.lap.zero <- function(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv) 
{       
  vtheta <- c(vbeta, vu)
  vg <- t(cbind(mX, mZ))%*%(vr*vy - vr*exp(cbind(mX, mZ)%*%vtheta)) - rbind(mSigmaBeta.inv%*%vbeta, mSigma.inv%*%vu)
  return(vg)
}

###############################################################################

mH.lap.zero <- function(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv) 
{
	# FIXME: Is this updated for zero-inflation?
...
fit.Lap.zero <- function(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv, debug=FALSE) 
{
    MAXITER <- 100
    vmu = c(vbeta, vu)
    
    for (ITER in 1:MAXITER) {
        f  <- f.lap.zero(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv) 
        vg <- vg.lap.zero(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv) 
        mH <- mH.lap.zero(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv)
        mLambda <- solve(-mH,tol=1.0E-99)
        vmu <- vmu + mLambda%*%vg
		# FIXME: This code isn't general.
        vbeta <- vmu[1:2]
        vu <- vmu[3:4]
        if (debug)
    ...
    f <- f.lap.zero(vbeta,vu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv) + 0.5*log(det(mLambda%*%bdiag(mSigmaBeta.inv, mSigma.inv)))
    if (debug)
		print(f)
...
f.G <- function(vmu,mLambda,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh) 
# Calculate the main part of the lower bound.
{
    ...
    f <- sum(vr*vy*vmu.til - vB0) - 0.5*(t(vbeta)%*%mSigmaBeta.inv%*%vbeta + t(vu)%*%mSigma.inv%*%vu)
...
f.GVA <- function(vmu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh,mR,Rinds,Dinds)
# Calculate lower bound.
{
   ...
   mLambda <- mR%*%t(mR)   
	...
	# Calculate f
   f <- sum(log(diag(mR))) + f.G.zero(vmu,mLambda,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh) 
...
vg.G <- function(vmu,mLambda,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,vB1) 
{
    vg <- t(cbind(mX,mZ))%*%(vr*vy - vB1) - mSigmaBeta.inv%*%vbeta - mSigma.inv%*%vu     
    return(vg)
}

###############################################################################

mH.G <- function(vmu,mLambda,vr,vy,mZ,mSigma.inv,vB2) 
{
    vw <-  vB2; dim(vw) <- NULL
...
vg.GVA.approx <- function(vmu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh,mR,Rinds,Dinds)
{
    n <- length(vy); P <- length(vu); d <- ncol(mZ)
		# Question: Why do we do this epsilon averaging trick?
		eps <- 1.0E-6
		f <- f.GVA.zero(vmu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh,mR,Rinds,Dinds)
		vg.approx <- matrix(0,P,1)
		for (i in 1:P) {
		   vup <- vu 
		   vup[i] <- vu[i] + eps
		   fp <- f.GVA.zero(vup,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh,mR,Rinds,Dinds)
		   
		   vum <- vu 
		   vum[i] <- vu[i] - eps
		   fm <- f.GVA.zero(vum,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh,mR,Rinds,Dinds)
...
vg.GVA <- function(vmu,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,gh,mR,Rinds,Dinds)
{
    ...
    vg <- 0*vu
    vg[1:(d1+d2)] <- vg.G.zero(vmu,mLambda,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,vB1) 

    mLambda.inv <- solve(mLambda,tol=1.0E-99)
    mH <- mH.G.zero(vmu,mLambda,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,vB2)
...
fit.GVA <- function(vbeta,vu,mLambda,vr,vy,mX,mZ,mSigmaBeta.inv,mSigma.inv,method,reltol=1.0e-8)
{
	#library(statmod)
	#N <- 15
    ...
    res <- optim(par=vu, fn=f.GVA.zero, gr=vg.GVA.zero,
        method=method,lower=-Inf, upper=Inf, control=controls,
        vr=vr,vy=vy,mX=mX,mZ=mZ,mSigmaBeta.inv=mSigmaBeta.inv,mSigma.inv=mSigma.inv,
