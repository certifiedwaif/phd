...
\begin{algorithm}\label{alg:updateGamma}
	...
	\begin{algorithmic}
		\medskip 
		\WHILE{$\log \underline{p}(\vy; \vw, \mGamma)$ is still different from the previous iteration}
		\medskip
			\FOR{$k = 1, \ldots, K$}
				\medskip
				\FOR{$j = 1, \ldots, p$}
...
% \begin{figure}[h!]
	...
	% \caption{The total posterior mass (TPM) and  mean marginal variable error (MMVE) for four data sets
...
For data sets with $p>45$ it is not feasible to perform exact BMA.  For these
...
\begin{figure}[b!]
	\includegraphics[width=0.6 \textwidth]{code/blma/cva_low_dimensional.pdf}
...
From the top panel of \ref{fig:commPVA_F1} we see that 
all model priors
...
From the top panel of \ref{fig:qtlPVA_F1_compare} we see that 
