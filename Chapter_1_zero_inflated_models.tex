...
\noindent The lower bound is equal to $\bE_q[\log{p(\vy, \vtheta)} - \log{q(\vtheta)}] = $T$_1 + $T$_2 + $T$_3$, with an additional term for $\sigma^2_u$. The T_2$ term is the same as in the univariate model.
...
$$
\begin{array}{rl}
$T$_3 &= \bE_q[\log{p(\sigma^2_u)} - \log{q(\sigma^2_u)}] \\
