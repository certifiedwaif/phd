...
Function
\begin{align*}
\vp^T\vy^T\mC\vmu - \vp^T\exp{(\mC \vmu + \half \mC \mLambda \mC^T)} - \half \vmu^T \mSigma^{-1} \vmu - \half \tr{(\mLambda \mSigma^{-1})}
\end{align*}

Derivative
\begin{align*}
% Should there be a \vp in front of \vy? There is in the above function
vg &= \mC(\vy - \vp^T \exp{(\mC \vmu)}) - \mSigma^{-1} \vmu \\
mH &= -\mC^T \text{diag}(\exp{(\mC \vmu)}) \mC - \mSigma^{-1}
\end{align*}
...
vg
\begin{align*}
\mC^T\vp[\vy - B^{1}(\mC \vmu, \text{diag}(\mC \mLambda \mC^T))] - \mSigma^{-1} \vmu
\end{align*}

mH
\begin{align*}
-\mC^T \text{diag}(\vp \odot B^{(2)}(\mC \vmu, \text{diag}(\mC \mLambda \mC^T))) - \mSigma^{-1}
\end{align*}

Algorithm
\begin{align*}
\mLambda \leftarrow (\mC^T\vp^T B^{(2)}(\mC \vmu, \text{diag}(\mC \mLambda \mC^T)) \mC)^{-1} \\
\vmu \leftarrow \vmu + \mLambda vg
\end{align*}
...
% This section is not very interesting.
