...
smoothing splines while remaining computationally efficient. We also present a new parameterisation for the
Gaussian Variational Approximation which provides increased speed and numerical stability while preserving
accuracy.
...
Variational Bayes is an approximate Bayesian inference method as detailed in \cite{ormerod10} and
\cite{RohdeWand2015}, which allows us to fit close approximations to these models using a deterministic
...
>>>>>>> f39f780f7603b1002422e14d6d33ab7aede5b3ea

\subsection{\joc{Variational Approximate Bayesian Inference (perhaps Semiparametric mean field Variational Bayes)}}
...
for a Poisson model, but the sample otherwise behaves like a Poisson sample. There are two main 
...
A second approach considers the data as the product of two processes, a binary process
...
An auxiliary variable representation of the second parameterization introduces the auxiliary
...
We can add covariates using a log-link where we replace $\lambda$ with $\vx_i^T\vbeta$ where
...
The $j$th predictor/response pair for the $i$th group is denoted by $(\vx_{ij}, \vy_{ij}), 1 \leq j \leq n_i, 1 \leq i \leq m$, where the $\vx_{ij}$ are unrestricted \joc{(what do you mean by this??)}, while the $\vy_{ij}$ are nonnegative integers.

$$1 \leq i \leq m$, where the $\vx_{ij}$ are real, while the $\vy_{ij}$ are nonnegative integers.$$


For each $1 \leq i \leq m$, define the $n_i \times 1$ vectors $\vy_{ij} = [\vy_{i 1}, \ldots, \vy_{i
n_i}]^\top$ and let $\vone$ define a vector of ones of appropriate length, where the first of these vectors is the response. It is
...
\vbeta|\sigma^2_\vbeta \sim \mbox{N}(\vzero, \sigma^2_\vbeta \mI)\\
\mbox{ and } \vu \sim \mbox{N}(\vzero, \mSigma_{\vu \vu})
\end{align*}
