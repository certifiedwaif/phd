...
\subsection{Variational Bayesian zero-inflated count models}

\section{A factorised variational approximation to the univariate zero-inflated Poisson model}

% TODO: Add graphical model

\noindent We use a factorised approximation to the full likelihood, as detailed in \citep{ormerod10}.
...
$$
q(\lambda, \rho, \vp) = q(\lambda) q(\rho) \left [ \prod_{i=1}^n q(r_i) \right ], \quad 1 \leq i \leq n
$$
...
\mbox{ and } q^*(r_i) \sim \text{Bernoulli}(p_{q(r_i)}), \mbox{1 \leq i \leq n},
$$

where the parameters are updated according to Algorithm \ref{algorithm1}.

\begin{algorithm}\label{alg:algorithm_one}
\label{algorithm1}
...
\noindent Assuming that $q(r_i) \sim \Bernoulli{p_i}$ then,

% Mean field update for q(\lambda)
$$
\begin{array}{rl}
q^*(\lambda) &\propto \lambda^{\alpha_\lambda+\vone^T\vx - 1} \exp{\left[ \bE_{-q(\lambda)} \left \{-(\beta_\lambda + \vone^T\vr) \lambda \right \} \right]} \\
...
$$
\begin{array}{rl}
\log{q^*(\rho)} &\propto \left \{ \bE_{-q(\rho)} \left[ \vone^T\vr \log{(\rho)} + (n - \vone^T\vr) \log{(1 - \rho)} \right] + \alpha_\rho \log{(\rho)} + \beta_\rho \log{(1 - \rho)} \right \} \\
&\propto \exp{ \left( (\vone^T\vp + \alpha_\rho) \log{(\rho)} + (n - \vone^T\vp + \beta_\rho) \log{(1 - \rho)} \right) } \\
...
&= \text{expit}(\eta_i), \mbox{ when } x_i = 0
\end{array}
$$

\noindent and $\eta_i = -\frac{\alpha_{q(\lambda)}}{\beta_{q(\lambda)}} + \psi(\alpha_{q(\rho)}) - \psi(\beta_{q(\rho)})$.
...
\noindent The lower bound can be calculated directly to be
...
\quad \bE_q[\log{q(\rho)}] &= - \{ \log{(\text{Beta}(\alpha_{q(\rho)}, \beta_{q(\rho)})} - (\alpha_{q(\rho)} - 1) \psi{(\alpha_{q(\rho)})} - (\beta_{q(\rho)} - 1)\psi{(\beta_{q(\rho)})} \\
&\quad + (\alpha_{q(\rho)} + \beta_{q(\rho)} - 2)\psi{(\alpha_{q(\rho)} + \beta_{q(\rho)})} \} \\
\end{array}
$$
...
\mbox{and } q(r_i) &\propto \exp{\left \{-r_i \bE_{-r_i} [\exp{(c_i^T\vnu)}] + r_i [\psi(\alpha_\rho) - \psi(\beta_\rho)] \right \} }.\\
\end{array}
$$
%\end{align*}

\noindent Hence $q(r_i) = \Bernoulli{p_i}$ where
$$
...
=&\vy^T\mR\mC \vmu - \vp^T \exp{\{\mC \vmu + \half \text{diag}(\mC \mLambda \mC^T)\}} - \half \vmu^T \text{diag}\left (\frac{\alpha_{q(\sigma_{\vu}^2)}}{\beta_{q(\sigma_{\vu}^2)}} \right ) \vmu - \half \text{tr}(\mLambda \text{diag}\left (\frac{\alpha_{q(\sigma_{\vu}^2)}}{\beta_{q(\sigma_{\vu}^2)}} \right ))
\end{array}
$$

%where $\hat{\mD} = \frac{\alpha_{q(\sigma_{\vu}^2)}}{\beta_{q(\sigma_{\vu}^2)}} \mI$. 
\noindent This can be seen to be very close to a Poisson regression model.
...
\noindent The lower bound is equal to $\bE_q[\log{p(\vnu)} - \log{q(\vnu)}] = T_1 + T_2 + T_3$, with an additional term for $\sigma^2_u$. The $T_1$ term is the same as in the univariate model.
...
$$
\begin{array}{rl}
T_2 &= \bE_q[\log{p(\vnu)} - \log{q(\vnu)}] \\
&= \vy^T \mP \mC \vmu - \vp^T \exp{\{\mC \vmu + \half \text{diag} (\mC^T \mLambda \mC)\}} - \vone^T\log \Gamma{(\vy + \vone)}\\
