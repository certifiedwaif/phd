...
where $q(\vtheta) = q(\lambda) q(\rho) \prod_{i=1}^n q(r_i)$,
...
\mbox{and } q(r_i) &\propto \exp{\left \{-r_i \bE_{-r_i} [\exp{(c_i^T\vnu)}] \right \} + r_i [\psi(\alpha_\rho) - \psi(\beta_\rho)]}.\\
\end{array}
$$
%\end{align*}
...
$$
\begin{array}{rl}
&\bE_{-q(\vr)} [\vy^T\mR(C\vnu) - \vr^T\exp{(\mC\vnu)}-\half \vnu^T \mD(\sigma_{\vu}^2)^{-1} \vnu]\\
...
\noindent The lower bound is equal to $\bE_q[\log{p(\vnu)} - \log{q(\vnu)}] = T_1 + T_2 + T_3$.
The $T_1$ term is the same as in the univariate model.
...
&= \vy^T \mP \mC \vmu - \vp^T \exp{\{\mC \vmu + \half (\mC^T \mLambda \mC)\}} - \vone^T\log \Gamma{(\vy + \vone)}\\
