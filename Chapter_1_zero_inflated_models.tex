...
We consider have a sample of counts $y_i$, $1\le i\le n$, where there are excessive number of zeros for a
...
\noindent The covariance of $\Cov(\vu) \equiv \blockdiag_{1 \leq i \leq m} (\mSigma) \equiv \mI_m \otimes \mSigma$.

\noindent In the random intercept case, $\mSigma = \sigma_u^2 \mI$ while in the random slopes case
$$\mSigma = 
\begin{pmatrix}
...
\[
	\mSigma =
	\begin{pmatrix}
	\sigma^2_{\text{intercept}} \\
		\rho_{\text{intercept} x} & \sigma^2_x \\
		\rho_{\text{intercept} x^2} & \rho_{x x^2} & \sigma^2_{x^2} \\
		\rho_{\text{intercept} x^3} & \rho_{x x^3} & \rho_{x^2 x^3} & \sigma^2_{x^3} \\
		0 & \rho_{x (x - \kappa_1)^3_+} & \rho_{x^2 (x-\kappa_1)_+^3} & \rho_{x^3 (x - \kappa_1)_+^3} & \sigma^2_{(x - \kappa_1)_+^3}
	\end{pmatrix}
\]
...
\begin{algorithm}
	...
	The GVA NP algorithm was used to fit a random intercept model to the Roaches data set provided by Andrew Gelman. The fitted co-efficients and accuracy results are presented in Table
