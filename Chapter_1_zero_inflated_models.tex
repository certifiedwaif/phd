...
Semiparametric mean field Variational Bayes is an approximate Bayesian inference method \cite{RohdeWand2015}.
The approximation is found by iteratively minimising the Kullback-Leibler divergence between the true posterior
...
&= \int q(\vtheta) \log \frac{p(\vy, \vtheta)}{q(\vtheta)} d \vtheta +
	 \int q(\vtheta) \log \frac{q(\vtheta)}{p(\vtheta|\vy)} d \vtheta \\
...
\noindent Thus the optimal approximation $q^*(\vtheta)$ is
...
\noindent Variational approximations are well-suited to accelerating the fit of Bayesian zero-inflated models
...
In this article, we compare the accuracy, stability and speed of a number of algorithms for fitting the 
...
log likelihood around the mode to find a Gaussian approximation of the full posterior.
This can then be optimised with Newton-Raphson style iterations. The algorithm is very
...
% Include diagram?
\noindent By re-ordering $\mC = [\mX \mZ]$ to $\mC = [\mZ \mX]$, and re-ordering the dimensions
...
\noindent The results for the random intercept, random slope and spline accuracy are presented in
...
\noindent The GVA2 algorithm was used to fit a random intercept model to the Roaches data set provided
...
\noindent The variational lower bound is equal to
