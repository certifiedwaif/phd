...
We employ priors $\rho \sim \Unif{(0, 1)}$ and $\lambda \sim \myGamma{(0.01, 0.01)}$. 
...
\noindent where $q(\vnu) = \mbox{N}(\vmu, \mLambda)$, $q(\sigma_{\vu}^2) = \mbox{Inv. Wishart}\left(v + m + d, d, \Psi + \sum_{i=1}^m \vmu_i \vmu_i^T + \mLambda_{{\vu}_i} \right)$ \mbox{and } $q(r_i) = \Bernoulli{(p_i)}$
...
\noindent where $\mD =  (v + m) \left( \Psi + \sum_{i=1}^m \vmu_i \vmu_i^T + \mLambda_{{\vu}_i} \right)^{-1}$. 
...
\STATE $\vp_{q(\vr_0)} \leftarrow \expit{(\eta)}$
% \mSigma_{\vu \vu}
\STATE $\Psi_{q(\mSigma_{\vu \vu})} \leftarrow \Psi + \sum_{i=1}^m \vmu_i \vmu_i^T + \mLambda_{{\vu}_i}$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
&= \vy^T \mP \mC \vmu - \vp^T \exp{\left [\mC \vmu + \half \text{diag} (\mC \mLambda \mC^T)\right ]} - \vone^T\log \Gamma{(\vy + \vone)}\\
...
&= v/2(\log |\Psi| - \log |\Psi + \vmu_\vu \vmu_\vu^T + \mLambda_{\vu \vu}|) + \half \log 2 + \half \bE_q \log |\mSigma_{\vu \vu}| + \log \Gamma_{p+1}(v/2) - \log \Gamma_{p}(v/2) \\
...
accuracy,  stability and speed. The Gaussian variational lower bound is
optimised using the L-BFGS-B  algorithm.

\subsubsection{GVA} % More complicated functions and derivatives, but more
accurate The first variant of the Gaussian Variational Approximation algorithm
...
be optimised with L-BFGS:

% Detail the function and its derivatives
\begin{align*}
\log \underline{p}(\vmu, \mLambda; \vy) &= \vy^T\mP \mC \vmu - \vp^T \B(\mC \vmu, \text{diag}(\mC \mLambda \mC^T)) - \half \vmu \mSigma^{-1} \vmu - \half \tr{(\mSigma^{-1} \mLambda)} \\
&\quad - \frac{d}{2} \log{(2 \pi)} + \half \log{|\mSigma^{-1}|} + \frac{d}{2} \log{(2 \pi)} + \frac{d}{2} + \log{|\mR|} \\
\frac{\partial \log \underline{p}(\vmu, \mLambda; \vy)}{\partial \vmu} &= \mC^T \vp^T(\vy - \B^{(1)}(\mC \vmu, \mC \mLambda \mC^T)) - \mSigma^{-1} \vmu \\
\frac{\partial \log \underline{p}(\vmu, \mLambda; \vy)}{\partial \mLambda} &= \left [\mLambda^{-1} - \vp^T \mC^T \text{diag}(\B^{(2)}(\mC \vmu, \mC \mLambda \mC^T)) \mC \vp) - \mSigma^{-1} \right ] \mR
\end{align*}
...
\noindent This new parameterisation allows us to solve $\mR \vx = \vc_i$ and then square
rather than performing the matrix multiplication $\vc_i \mLambda \vc_i$ as we do in the GVA
variant of the algorithm.

% TODO: The above computational complexity results may well be true, but I wouldn't trust them.
% It's been a long time since that thinking was done, and some of it was most likely wrong.
% The sparse matrix experiments that I've done generally haven't panned out very well.	
...
&= \quad(\mLambda^{-1} + \mH)(-\mLambda \mR \mLambda) \\
...
% Detail the function and its derivatives
\begin{align*}
\frac{\partial \log \underline{p}(\vmu, \mLambda; \vy)}{\partial \vmu} &= \quad \mC^T\vp \left [\vy - \B^{1}(\mC \vmu, \text{diag}(\mC \mLambda \mC^T)) \right ] - \mSigma^{-1} \vmu \\
...
Mean of $\vu$ & $0.951$ & $0.965$ & $0.958$ & $0.956$ & $0.965$ \\
