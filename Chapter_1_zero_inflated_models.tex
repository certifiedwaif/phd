...
q(\vnu) &= N(\vmu, \mLambda) \\
q(\sigma_{\vu}^2) &= IG(A + \frac{m}{2}, B + \frac{\|\mu_u\|^2}{2} + \frac{tr(\Sigma_{uu})}{2}) \\
q(r_i) \propto \exp{\{\bbE_{-r_i} [-r_i \exp{(c_i^T\vnu)}]\} + r_i [\digamma(\alpha_\rho) - \digamma(\beta_rho)]} \\
\end{align*}
...
i=&y^TRC \vnu - p^T \exp{\{C \mu + \text{dg}(C \mLambda C^T)\}} - \half \mu^T \hat{D} \mu - \half \text{tr}(\mLambda \hat{D})
\end{align*}
