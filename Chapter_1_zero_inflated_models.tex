...
&= \bE_q [v/2(\log |\Psi| - \log |\Psi + \vmu_\vu \vmu_\vu^\top + \mLambda_{\vu \vu}|) + \half \log 2 + \half \log|\mSigma_{\vu \vu}| \log \Gamma_{p+1}(v/2) - \log \Gamma_{p}(v/2)\\
&\quad + \half \tr((\vmu_{\vu} \vmu_{\vu}^\top + \mLambda_{\vu \vu}) \mSigma_{\vu \vu}^{-1}) ] \\
&= v/2(\log |\Psi| - \log |\Psi + \vmu_\vu \vmu_\vu^\top + \mLambda_{\vu \vu}|) + \half \log 2 + \half \bE_q \log |\mSigma_{\vu \vu}| + \log \Gamma_{p+1}(v/2) - \log \Gamma_{p}(v/2) \\
&\quad + \half \tr(\mI_m + \Psi(\Psi+ \vmu_\vu \vmu_\vu^\top + \mLambda_{\vu \vu})^{-1}/(v + p + 2)) \\
...
$$
\bE_q [\log |\mSigma_{\vu \vu}|] = m \log 2 + \log \left | \Psi + \vmu_\vu \vmu_\vu^\top + \mLambda_{\vu \vu} \right | + \sum_{i=1}^m \Psi \left ( \frac{v - i + 1}{2} \right )
$$
...
% Detail the function and its derivatives
\begin{align*}
\log \underline{p}(\vmu, \mLambda; \vy) &= \vy^\top\mP \mC \vmu - \vp^\top \B(\mC \vmu, \text{diag}(\mC \mLambda \mC^\top)) - \half \vmu \mSigma^{-1} \vmu - \half \tr{(\mSigma^{-1} \mLambda)} + \log{|\mR|} \\
...
optimisation methods such as L-BFGS-B work well.
...
\begin{align*}
\log \underline{p}(\vmu, \mLambda; \vy) &= \vy\mP\mC \vmu - \vp^\top \B(\mC \vmu, \text{diag}(\mC \mLambda \mC^\top)) - \half \vmu \mSigma^{-1} \vmu - \half \tr{(\mSigma^{-1} \mLambda)} \\
...
\hline
$\vbeta_1$ & $0.955$ & $0.961$ & $0.965$ & $0.961$ \\ 
$\vbeta_2$ & $0.790$ & $0.878$ & $0.857$ & $0.878$ \\ 
Mean of $\vu$ & $0.951$ & $0.965$ & $0.958$ & $0.965$ \\
$\sigma^2_\vu$ & $0.961$ & $0.963$ & $0.962$ & $0.963$ \\ 
$\rho$ & $0.893$ & $0.893$ & $0.891$ &  $0.893$ \\ 
\hline
\end{tabular}
\end{table}
...
Mean of $\vu$        &0.869&0.991&0.952&0.966&0.991\\
