...
$$
\begin{array}{ll}
q(r) &\propto \exp{\{\bE_{-q(\vr)}y^T\mR(C\vmu) - r^T\exp{(C\vnu)}-\half \vnu^T D(\sigma_{\vu}^2)^{-1} \vnu\}}
\end{array}
$$
...
=&y^TRC \vmu - p^T \exp{\{C \mu + \half \text{diag}(C \mLambda C^T)\}} - \half \mu^T \hat{D} \mu - \half \text{tr}(\mLambda \hat{D})
\end{array}
$$
...
+ \half (|2\pi \Lambda| + \vnu^T \Lambda^{-1} \vmu)
$$
