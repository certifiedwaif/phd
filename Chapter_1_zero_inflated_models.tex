...
zeroes. After introducing the representation of Bayesian zero-inflated models we consider and a short summary
...
respectively. Let $\vy$ by the $n \times 1$ vector. The norm of a column vector $\vv$, defined to be
$\sqrt{\vv^\top \vv}$, is denoted by $\|\vv\|$. For a $p \times 1$ vector $\va$, we let $\diag{(\va)}$ denote
the $p \times p$.

Let $\vtheta$ be the parameter vector in $\R^p$.

Let $\mX$ be a matrix with dimensions $n \times p$.
Let $\mZ$ be a matrix with dimensions $n \times m b$
Let $\mC = [ \mX \mZ ]$.
Let $\vbeta$ be the $p \times 1$ column vector representing the fixed effects.
Let $\vu$ be the $m b \times 1$ column vector representing the random effects.
Let $\vnu = [\vbeta^\top, \vu^\top]$.
