...
$$
\begin{array}{ll}
q^*(\rho) &\propto \exp{\{ E_{-q(\rho)} \left[ \vone^T\vr \log{(\rho)} + \vone^T(\vone - \vr) \log{(1 - \rho)} \right] } \\
...
a \log{(b)} + (a - 1) \bE[\log{(\lambda)}] - b \bE [\lambda] - \log{(\Gamma(a))} \\
&+ \sum_{i=1}^n  ( -\bE [\lambda] \bE [r_i] + \bE[x_i \log{(\lambda r_i)}] - \log{(x_i!)} \\
&+ \bE[r_i] \bE[\log{(\rho)}] + \bE[(1 - r_i)] \bE[\log{(1 - \rho)}] ) \\
& - \bE \log{q(r)} - \bE \log{q(\lambda)} - \bE \log{q(\rho)}
\end{array}
$$

where
$$
\bE [\log{\lambda}] = -\{ a_{q(\lambda)} - \log{(b_{q(\lambda)})} + \log{\Gamma(a_{q(\lambda)})} + (1 - a_{q(\lambda)}) \psi{(a_{q(\lambda)})} \} \\
$$
$$
\bE [\lambda] = \frac{a_{q(\lambda)}}{b_{q(\lambda)}} \\
$$
$$
\bE[\log{\rho}] = - \{ \log{B(a_{q(\rho)}, b_{q(\rho)})} - (a_{q(\rho)} - 1) \psi{(a_{q(\rho)})} - (b_{q(\rho)} - 1)\psi{(b_{q(\rho)})} + (a_{q(\rho)} + b_{q(\rho)} - 2)\psi{(a_{q(\rho)} + b_{q(\rho)})} \} \\
$$
...
$$
-\bE_q \log{q(\lambda)} = q_q^* - \log{(b_q^*)} + \log{\Gamma{(a_q^*)}} + (1 - a_q^*) \Psi{(a_q^*)}
$$

$$
\bE[\log{q(\rho)}] = - \{ \log{(\text{Beta}(a_q^*, b_q^*)} - (\alpha_q^* - 1) \Psi{(a_q^*)} - (\beta_q^* - 1)\Psi{(\beta_q^*)} + (\alpha_q^* + \beta_q^* - 2)\Psi{(\alpha_q^* + \beta_q^*)} \}
$$
...
$$
\begin{array}{ll}
\log{p(y|\vr, \vbeta, \vu)} &= \vy^T R (C\vnu) - \vr^T \exp{(C\vnu)} - 1^T \log{\Gamma{(y + 1)}} \\
\log{p(\vnu|\sigma_{u}^2)} &= - \half \log{|2\pi D(\sigma_{\vu}^2)|} - \half \vnu^T(\sigma_{\vu}^2 I)^{-1} \vnu \\
...
\begin{array}{rl}
q(\vnu) &= N(\vmu, \mLambda) \\
q(\sigma_{\vu}^2) &= IG\left(\alpha_{\sigma_u^2} + \frac{m}{2}, \beta_{\sigma_u^2} + \frac{\|\mu_u\|^2}{2} + \frac{\tr(\Lambda_{uu})}{2}\right) \\
...
$$
\begin{array}{ll}
q(r) &\propto \exp{\{\bE_{-q(\vr)}y^TR(C\vnu) - r^T\exp{(C\vnu)}-\half \vnu^T D(\sigma_{\vu}^2)^{-1} \vnu\}}
\end{array}
$$

$$
\begin{array}{ll}
&\bE_{-q(\vr)} [y^TR(C\vnu) - r^T\exp{(C\vnu)}-\half \vnu^T D(\sigma_{\vu}^2)^{-1} \vnu]\\
=&y^TRC \vnu - p^T \exp{\{C \mu + \text{diag}(C \mLambda C^T)\}} - \half \mu^T \hat{D} \mu - \half \text{tr}(\mLambda \hat{D})
\end{array}
$$

where $\hat{D} = D(A_{q(\sigma_{\vu}^2)} / B_{q(\sigma_{\vu}^2)}) I$. This can be seen to be very close to a Poisson
regression model.
...
% with \vbeta
$$
E_q [\log{p(y, \theta)} - \log{q(\theta)}] = \vy \vp^T X\vnu - \vp \exp{(X\vnu + \half \vnu^T \Lambda \vnu)} - \log{(vy + 1)}
+ \half (|2\pi \Lambda| + \vnu^T \Lambda^{-1} \vnu)
$$

For $\sigma^2_u$,
$$
E_q [\log{p(\sigma_u^2)} - \log{q(\sigma_u^2)}] = \frac{m}{2} E_q [\log{\sigma_u^{-2}}] - (\half \|\mu_u\|^2 + \tr(\Lambda_{uu})) E_q [\sigma_u^{-2}] - \log{\Gamma(\alpha_{\sigma_u^2})} + \log{\Gamma(\alpha_{\sigma_u^2} + \frac{m}{2} - 1)}
$$

where
$$
E_q[\log{(\sigma_u^2)}] = - \{\alpha_{\sigma_u^2}^2 - \log{\beta_{\sigma_u^2}^*} + \log{\Gamma(\alpha_{\sigma_u^2}^*)} + (1-\alpha_{\sigma_u^2}^*)\digamma{(\alpha_{\sigma_u^2}^*)}\}
$$

and
$$
E_q[\sigma_u^{-2}] = \frac{\alpha_{\sigma_u^2}^*}{\beta_{\sigma_u^2}^*}
$$
