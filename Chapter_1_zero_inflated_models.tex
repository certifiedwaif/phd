...
where $\eta_i = - \frac{\alpha_\lambda_{q(\lambda)}}{\beta_{q(\lambda)} + \Psi(\alpha_{q(\rho)}) - \Psi(\alpha_{q(\rho)})$.
...
\begin{array}{rl}
q(\vnu) &= N(\vmu, \mLambda) \\
q(\sigma_{\vu}^2) &= IG\left(A + \frac{m}{2}, B + \frac{\|\mu_u\|^2}{2} + \frac{tr(\Lambda_{uu})}{2}\right) \\
q(r_i) &\propto \exp{\{\bE_{-r_i} [-r_i \exp{(c_i^T\vnu)}]\} + r_i [\digamma(\alpha_\rho) - \digamma(\beta_\rho)]} \\
\end{array}
$$
%\end{align*}
