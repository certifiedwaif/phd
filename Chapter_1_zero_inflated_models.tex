...
% The approach that I'm taking is to make use of the trick that John suggested, which is to recognise that
% the lower bound of the variational approximation is the KL divergence between the full likelihood p and
% the factor approximation q.
$$
...
$$
% TODO: Substitute current values of a_{q(\rho)} and b_{q(\rho)} for a_* and b_*

$$
KL(q_{\lambda}^* || p(\lambda)) = (a_* - a) \Psi(a_*) - \log{\Gamma(a_*)} + \log{\Gamma(a)} + a(\log{b_*} - \log{b}) + \frac{a_*(b_* - b)}{b_*} 
$$
% TODO: Substitute current values of a_{q(\lambda)} and b_{q(\lambda)} for a_* and b_*

% Now we come to what I think will be the sticking point: p(x_i | r_i, \lambda, \rho)
% I think that this is intractable, or will have to be approximated. But we'll see what John says.
$$
...
$$
\half \log{(2 \pi e \lambda_*)} - \frac{1}{12 \lambda_*} - \frac{1}{24 \lambda_*^2} - \frac{19}{360 \lambgda_*^3} + O(\frac{1}{\lambda_*^4})
$$
