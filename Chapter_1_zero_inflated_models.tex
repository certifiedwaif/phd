...
Maximise the variational lower bound with respect to $\vmu, \mLambda$, keeping $\vp, \mSigma, \rho$ fixed. The
difficulty is in the maximisation of $\mLambda$ - the dimension of the covariance matrix is $p^2$, and the
space of covariance matrices is constrained.
Optimise Cholesky factor or Cholesky factor of inverse, ensuring symmetry.
Use knowledge of the regression model to specify a sparse matrix structure, greatly reducing the 
dimension of the problem.
Parameterise diagonals with $\exp{(d_ii)} / \exp{(-d_ii)}$ to ensure unique positive definiteness.
This parameterisation can lead to overflow, which we dealt with by thresholding.
Over can lead to singular matrices when attempting to invert $\mLambda$. This problem was mitigated by 
instead working with the Cholesky factorisation of $\mLambda^{-1}$, allowing us to solve rather than
invert and multiply, which is also faster and more numerically stable.
Different ways to calculate the gradient.
