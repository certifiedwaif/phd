...
\frac{\partial \log p(\vmu, \mLambda; \vy)}{\partial \mLambda} &\approx -\mC^T \text{diag}(\exp{(\mC \vmu)}) \mC - \mSigma^{-1}
\end{align*}
...
will be 0 and so the Cholesky factor $\mR$ will be sparse. This mitigates the increased
