...
(\alpha_{\sigma^2_u} - 1) \bE_q	\log{\sigma^2_u} - \beta_{\sigma^2_u} \bE_q [\sigma^2_u] +
\log{\Gamma(\alpha^*_{\sigma^2_u})} - (\alpha^*_{\sigma^2_u} - 1) \Psi{(\alpha^*_{\sigma^2_u})} - \log{(\beta^*_{\sigma^2_u})} - \log{(\alpha^*_{\sigma^2_u} + \beta^*_{\sigma^2_u})}\\
=\alpha_{\sigma^2_u} \log{\beta_{\sigma^2_u}} - \log{\Gamma{(\alpha_{\sigma^2_u})}} +
(\alpha_{\sigma^2_u} - 1) \Psi{\alpha^*_{\sigma^2_u}} - \log{(\beta^*_{\sigma^2_u})} - \beta_{\sigma^2_u} \frac{\alpha^*_{\sigma^2_u}}{\beta^*_{\sigma^2_u}} + \\
\log{\Gamma(\alpha^*_{\sigma^2_u})} - (\alpha^*_{\sigma^2_u} - 1) \Psi{(\alpha^*_{\sigma^2_u})} - \log{(\beta^*_{\sigma^2_u})} - \log{(\alpha^*_{\sigma^2_u} + \beta^*_{\sigma^2_u})}
$$

where

$\alpha^*_{\sigma^2_u} = \alpha_{\sigma^2_u} + \frac{m}{2}$,
$\beta^*_{\sigma^2_u} = \beta_{\sigma^2_u} + \frac{\|\vmu_\vu\|^2}{2} + \frac{\tr{(\mLambda_{\vu \vu})}}{2}$ \subsection{Results}
