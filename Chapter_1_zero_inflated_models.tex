...
p(\rho) &\propto 1, \\
\mbox{and } \log{p(\sigma^2)} &= A \log{(B)} - \log{\Gamma{(A)}} - (A + 1) \log{(\sigma_{\vu}^2)} - \frac{B}{\sigma_{\vu}^2}\\
\end{array}
$$
...
q(\sigma_{\vu}^2) &= \mbox{IG}\left(A + \frac{m}{2}, B + \frac{\|\vmu_\vu\|^2}{2} + \frac{\tr(\mLambda_{\vu\vu})}{2}\right), \\
...
\noindent where
$$
p_i = \expit\left[ \psi{(\alpha_q)} - \psi{(\beta_q)} - \exp{(c_i^T\vmu + \half c_i^T \mLambda c_i)} \right]
$$

\noindent where $\hat{\mD} = \text{diag}\left ( A_q/B_q \right )$,
and $\alpha_q = \alpha_\rho + \vone^T\vp$ and 
$\alpha_q = \beta_\rho + \vone^T(\vone - \vp)$ \text{when} $\vy_i = 0$.
...
optimal densities $q^*(\vmu, \mLambda)$, $q^*(\sigma_u^2)$, $q^*(\vr)$ and $q^*(\rho)$}
\begin{algorithmic}
% Fit \vmu, \mLambda using Laplace approximation
\REQUIRE{$A_q \leftarrow A + m/2$}
\WHILE{the increase in $\log{\underline{p}}(\vy;q)$ is significant}
% \vmu, \mLambda
\STATE Optimise $\vmu$ and $\mLambda$ using $\vy, \mX, \mZ, \rho, \vp$ and
$A_q$, $B_q$
% \rho is not directly observed, except through \vr_i
\STATE $\alpha_q \leftarrow \alpha_\rho + \vone^T\vp$
\STATE $\beta_q \leftarrow \beta_\rho + n - \vone^T\vp$
% \vp
\STATE $\eta \leftarrow -\exp [\mC \vmu + \half \diag{(\mC\mLambda\mC^T)}] + \psi{(\alpha_q)} - \psi{(\beta_q)}$
\STATE $\vp_{q(\vr_0)} \leftarrow \expit{(\eta)}$
% sigma_u^2
\STATE $B_q \leftarrow B + \half \|\vu\| + \half \tr(\mLambda_{\vu \vu})$
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
& \quad - \frac{p}{2} \log{(\sigma_\vbeta^2)} - \frac{\|\vmu_\vbeta\|^2 + \tr{(\mLambda_{\vbeta \vbeta}})}{2 \sigma_{\vbeta}^2} + A \log{(B)} - \log{\Gamma(A)} \\
&\quad - A_q \log{(B_q)} + \log{\Gamma(A_q)}
+ \frac{p+m}{2} + \half \log{|\mLambda|}
\end{array}
$$
...
&=A \log{B} - \log{\Gamma(A)} \\
&\quad - A_q \log{(B_q)} + \log{\Gamma(A_q)} \\
&\quad - (A - A_q) [\psi(A_q) - \log{(B_q)}] - (B - B_q) \frac{A_q}{B_q}
\end{array}
$$
...
& \ds \quad + \sum_{i=1}^n ( -\bE_q [\lambda] \bE_q [r_i] + \bE_q[x_i \log{(\lambda r_i)}] - \log\Gamma(x_i+1)) \\
& \ds =
...
- \bE_q[\log{q(\lambda)}] 
- \bE_q[ \log{q(\rho)}] \\
&= \quad \vp^T[\psi{(\alpha_{q(\lambda)})} - \psi{(\alpha_{q(\lambda)} + \beta_{q(\lambda)})}] + (\vone - \vp)^T[\psi(\beta_{q(\lambda)}) - \psi{(\alpha_{q(\lambda)} + \beta_{q(\lambda)})}] \\
& \quad - \vp^T \log{\vp} - (\vone - \vp)^T \log{(\vone - \vp)} \\
& \quad + \alpha_{q(\lambda)} - \log{\beta_{q(\lambda)}} + \log \Gamma{(\alpha_{q(\lambda)})} + (1 - \alpha_{q(\lambda)}) \psi{(\alpha_{q(\lambda)})} \\
& \quad + \log{\text{Beta}}(\alpha_{q(\lambda)}, \beta_{q(\lambda)}) - (\alpha_{q(\lambda)} - 1) \psi{(\alpha_{q(\lambda)})} - (\beta_{q(\lambda)} - 1) \psi{(\beta_{q(\lambda)})} + (\alpha_{q(\lambda)} + \beta_{q(\lambda)} - 2) \psi{(\alpha_{q(\lambda)} + \beta_{q(\lambda)})}
\end{array}
$$
