...
\begin{algorithm}
  ...
  $\frac{p(p-1)}{2}$ parameters to deal with instead of $p^2$ parameters.
	...
	+ 1)$ floating point operations, and storing the results of the $i$th product in the $i$th element of the
	vector $\va$ and then calculate $\diag(\mC \mLambda \mC^\top) = \va^\top \va$.

	Moreover, by exploiting our knowledge of the model, we can encode $\mR$ as a sparse matrix, and further reduce
	...
	$n \times [m + \half{1}{2} p (p + 1) + p \times m b]$ floating point operations.
	...
	we can calculate $\diag(\mC \mLambda \mC^\top)$.
	...
	computationally expensive and numerically unstable process taking $O(p^3)$ flops, and from constructing and 
