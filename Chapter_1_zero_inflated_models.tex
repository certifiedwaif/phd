	...
	\begin{algorithmic}
		...
		\STATE $\eta \leftarrow -\exp \left [ \mC \vmu + \half \diag{(\mC\mLambda\mC^\top)} \right ] + \psi{(\alpha_{q{(\rho)}})} - \psi{(\beta_{q{(\rho)}})}$ \\[1ex]
			...
			\begin{align*}
				y_i &= \begin{cases}
				0 if R_i = 0 \\
				\Poisson(e^{\mX_i \vbeta + \mZ_i \vu}) \phantom{-} \text{if} R_i = 1,
				\end{cases} \\
			...
			The model fit was
			\[
			y_i = R_i e^{\vbeta_1 + \vbeta_2 \text{female} + \vbeta_3 \text{married} + \vbeta_4 \text{children under age 6} + \vbeta_5 \text{PhD}}
			\]
			with priors
			\begin{align*}
			R_i &\sim \Bernoulli(\rho), \\
			...
			\noindent with $A=1$, $B=1$ and $\sigma_\vbeta^2 = 10,000$. The model was fit using the GVA inverse parameterisation algorithm.
			...
			fit in the model were FoodTreatment (Deprived or Satiated) and ArrivalTime, a continuous covariate.
			...
			\]
			where $n$ is the nest number, $1 \leq n \leq 25$, with priors
			\begin{align*}
			R_i &\sim \Bernoulli(\rho), \\
			\rho &\sim \Beta(A, B), \\
			...
			\end{align*}
			where $\sigma_\vbeta^2=10,000$, $A=1$, $B=1$, $s=1$ and $r=10^{-5}$.
