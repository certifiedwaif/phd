...
\begin{frame}
	...
	\begin{itemize}
		...
		\item A lot of work has been done on specifying
		parameter priors for Bayesian model selection.
		...
		\item We derive several new exact closed form expressions for the parameter posteriors for
		a popular choice of priors for linear models.
		
		\item These expressions are in terms of 
		special functions and  are numerically well-behaved, accurate, and fast to evaluate.		
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\end{align*}
	where $a = 1$, $b = p$, $c=(n - 1)/2$ and $d = c - b - 2 = p / 2 + a$.
\end{frame}
