...
\begin{equation}
	...
	\end{array} 
\end{equation}
\noindent where $\widehat{\vbeta}_{\mbox{\tiny LS}} = \left( \mX^T\mX \right)^{-1}\mX^T\vy$. \mgc{Should we say something about being unable to get the posterior distribution of $\vbeta$ in closed form?}
...
Fourth, recalling that $\tau_{\sigma^2} = \E_q [\sigma^{-2}] = r/s = (n+p)/(n[1 - (1 + \tau_g)^{-1} R^2] + p \tau_{\sigma^2}^{-1})$, we can solve for $\tau_{\sigma^2}$ in terms of $\tau_g$ to obtain
\[
