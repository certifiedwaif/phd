...
where the prior on $g$ is Zellner's g-prior on the covariance matrices \cite{Goel1986}, as this yields a
tractable posterior for $\vbeta$ as shown by \citep{Liang2008}. We choose $a$  and $b$ to be $-3/4$ and $(n -
p)/2 - a - 2$ respectively, following \citep{Maruyama2011}.
\mgc{Why?}
The prior on $\sigma^2$ is the Jeffrey's prior, as discussed in \cite{Berger2012}.
...
section of the parameter space where the data is least informative, makes the prior scale-invariant

We consider a linear model with a g-prior, following \citep{Maruyama2011} and \citep{Liang2008}. 
...
visited. As we traverse the space of models $\vgamma$, we calculate
\[
