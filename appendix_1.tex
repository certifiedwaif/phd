...
The variational lower bound is $\bE_q\{\log{p(\vy, \vtheta)} - \log{q(\vtheta)}\} = T_1 + T_2 + T_3$,
where
% This is the new T_1
\begin{equation*}
\begin{array}{rl}
...
\subsection{Derivatives for  parameterisation $\mLambda = \mR \mR^\top$}
...
\subsection{Derivatives for fixed point approach}
