...
\def \BigO {\text{O}}
rtp: $R_{X, Z} = R^2_{X} + \BigO(w^*)$ where $w^* = \max{\vw}$.
...
\leq & \| \vy\top \vy \| (p_\mX + p_\mZ + p_\mZ \frac{w_*^{-1} - 1}{\cancel{1} + w_*^{-1} - \cancel{1}}) \\
=& p_\mX + 2 p_\mZ - p_\mZ (1 - w_*) \text{ Linear function of } w_*
\end{array}
\end{equation*}
