...
search the model space in $\BigO(2^p np^2)$ rather than $\BigO(2^p np^3)$. Applications to \ldots are
presented, showing empirically that our method performs well on real-world data. Our method is
computationally more efficient than the exact Bayesian model.

\section{Introduction}

% Problem in general
The problem of model selection is one of the most important statistical problems encountered in practice by
...
parameter, instead allowing the specification of a prior distribution on that parameter.
