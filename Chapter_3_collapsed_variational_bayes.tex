...
\]
\begin{proof}
% I've done it on paper. I'll type it up tomorrow. Note that $\mA$ need not be symmetric, although $\mB$
% must be.

\begin{equation*}
\begin{align*}
(A \odot B)(A^{-1} \odot \frac{1}{B})_{ij} &= \sum_{k=1}^n A_{ik} B_{ik} A^{-1}_{kj} \frac{1}{B_{kj}} \\
&= \begin{cases}
1 B_{ik} \frac{1}{B_{kj}} \text{ when i=j, for B symmetric} \\
0 \text{ otherwise}
\end{cases} \\
&= \sum_{i=1}^n A_{ik} B_{ik} A^{-1}_{kj} \frac{1}{B_{kj}} \\
&= \sum_{k=1}^n \cancel{B_{ik}} \frac{1}{\cancel{B_{ik}}} A_{ik} A^{-1}_{kj}, i = j \\
&=\sum_{k=1}^n A_{ik} A^{-1}_{kj} B_{ik} \frac{1}{B_{kj}} \\
&=
\begin{cases}
\sum_{k=1}^n \cancel{B_{ik}} \frac{1}{\cancel{B_{ik}}} A_{ik} A^{-1}_{kj}, i = j \\
\sum_{i=1}^n A_{ik} A^{-1}_{kj} B_{ik} \frac{1}{B_{kj}}
= B_{i.} \odot (A^T_{i.} A^{-1}_{.j}) \odot \frac{1}{B_{.j}}
= B_{i.} \odot (0) \odot \frac{1}{B_{.j}}, i \ne j
\end{cases}
\end{align*}
\end{equation*}
\end{proof}
