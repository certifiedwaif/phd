...
&= {-\left(\frac{n + p}{2} + 1\right) \log \sigma^2 -  \frac{1}{\sigma^2} \frac{1}{2} \E_{q(\vbeta)} \left[  \left(1 + \frac{1}{g} \right) \vbeta^\top \mX^\top \mX \vbeta - \vbeta^\top \mX^\top \vy \right]} \\
&= -\left(\frac{n + p}{2} + 1 \right)\log \sigma^2 - \frac{1}{\sigma^2} \frac{1}{2} \left\{ \left( 1 + \tau_g \right) [\vmu_{q(\vbeta)}^\top \mX^\top \mX \vmu_{q(\vbeta)} + \tr(\mX^\top \mX \mSigma_{q(\vbeta)})] + \vy^\top \mX \vmu_{q(\vbeta)} \right\} \\
&= -\left(\frac{n + p}{2}\right) \log \sigma^2 - \frac{1}{\sigma^2} \frac{1}{2} \{ \cancel{(1 + \tau_g)}[(1 + \tau_g)^{-1} \vy^\top \mX^\top \cancel{(\mX^\top \mX)^{-1}}  \cancel{\mX^\top \mX} \cancel{(1 + \tau_g)^{-1}} (\mX^\top \mX)^{-1} \mX^\top \vy \\
&\quad+ \tr{(\tau_{\sigma^2} \cancel{\mX^\top \mX} \cancel{(\mX^\top \mX)^{-1}})}] + (1 + \tau_g)^{-1} \vy^\top \mX (\mX^\top \mX)^{-1} \mX^\top \vy \} \\
&= -\left(\frac{n + p}{2}\right) \log \sigma^2 - \frac{1}{\sigma^2} \{ [(1 + \tau_g)^{-1} + 1]\|\vy\|^2 R^2
		+ p \tau_{\sigma^2}^{-1} \}
\end{align*}
...
&= -\half \log g - \frac{1}{g} \half \E \frac{1}{\sigma^2} \tr[(\mSigma_{q(\vbeta)} + \vmu_{q(\vbeta)} \vmu_{q(\vbeta)}^T) \mX^\top \mX] + b \log g - [(a + 1) + (b + 1)] \log(1 + g) - \log \Beta(a + 1, b + 1)] \\
\end{align*}
