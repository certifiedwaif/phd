...
AIC (Akaike \cite{DeLeeuw1992}), BIC (), Mallow's $C_p$ and DIC. As Breiman \cite{Breiman1996} and Efron
...
Consider a normal linear model on $\vy$ with conjugate normal prior on \vbeta with mean centred at \vzero, and
covariance $g \sigma^2 \mX^\top \mX)^{-1}$ where the prior on $g$ is Zellner's g-prior on the covariance
