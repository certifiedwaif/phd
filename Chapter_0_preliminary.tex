...
and comprehensive introduction to the subject of multivariate optimisation is given in \cite{Nocedal2006}.

% TODO: This is technically correct, but so terse. Rewrite.
...
be closer to the  stationary point $\vtheta^*$ of $f$. This choice of $\vtheta_{n+1}$ establishes an iterative
algorithm for finding the  maxima/minima of $f$.

We continue until the step length $\| \vtheta_{n+1} - \vtheta_{n} \| < \epsilon$, for some suitably small
choice of $\epsilon$, such as $\epsilon = 10^{-8}$, indicating that convergence has been achieved.
