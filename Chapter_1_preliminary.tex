...
Approximation using the Cholesky factor of the precision matrix, similar to
...
models to non-linear situations is needed that still retains the beneficial
...
polynomial of degree $< n$ between and including adjacent knots. A B-Spline is
...
Or the process of model selection can be made implicit in the model fitting
process itself, ridge regression \citep{Casella1980}, of which the well-known
lasso is a special case \citep{Tibshirani1996}. As \citep{Breiman1996} and
\citep{Efron2013} showed, while  the standard formulation of a linear model is
...
Variational approximation can be viewed as minimising the Kullback- Leibler
...
difficult algebra to complete the derivations required for the updates, and is
slow to compute. Laplace approximation relies on a Gaussian approximation to
...
\item Exact inference for model selection for linear models with normal priors
    ...
    fourth chapter, we propose a population-based algorithm, which works by
    ...
    \citep{Bayarri2012} and the Maruyama and George Beta-Prime
    \citep{Maruyama2011} prior.
