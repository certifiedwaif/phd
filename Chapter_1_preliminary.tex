...
Zellner's g-prior, first introduced in \cite{Zellner1986},

\[
	\vbeta_\vgamma | \sigma^2, \mathcal{M}_\vgamma \sim \N(\vzero, g \sigma^2 (\mX^\top \mX)^{-1})
\]

is widely used for variable selection in linear models with normal priors owing to its'
computational efficiency in evaluating marginal likelihoods and model selection and
conceptually simple interpretation.
which scales the Fisher information $\sigma^2 (\mX^\top \mX)^{-1}$ by $g$,

This model specification performs shrinkage on the regression co-efficients,
and so has the advantage of performing well on model selection problems where the true model is sparse in
the sense that the number of true non-zero covariates $p$ is less than the number of samples $n$.
As stated in \cite{Hastie2015}, it is best to `best on sparsity': ``Use a procedure that does well in sparse 
problems, since no procedure does well in dense problems.''.

With different choices of hyper-prior on $g$, this shrinkage can be made to behave like Bayesian versions of
the lasso, ridge regression or generalisations of these.
...
Thus we have approximated our integral by a closed form expression. The error in the approximation is
$O(1/M)$. The approximation can be made more accurate by using a Taylor expansion beyond second order.
