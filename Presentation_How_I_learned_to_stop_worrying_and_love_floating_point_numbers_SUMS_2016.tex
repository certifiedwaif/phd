...
\end{itemize}
\item Finite memory, a 1 dimensional array of bytes
\item All operations take time to complete
\end{itemize}
\end{frame}
...
\small
\begin{itemize}
\item Bill Gosper came up with algorithms to perform ``exact'' arithmetic on continued fractions in 1972.
\item Continued fraction representations exist for all real numbers.
\item You can move between continued fraction, power series and rational function representations of 
			functions.
\item There are big advantages to using this representation for some things. The continued fraction 
			representation of a function or real number can converge more quickly than the power series 
			representation. And you only need to supply terms as they are needed.
\item Calculating the square root of two is all very well. The problem comes when you try to go the other
			way. What is $[1; 2, 2, 2, \ldots] \times [1; 2, 2, 2, \ldots]$?
\item Gosper'’s mul­ti­plica­tion scheme will run for­ever with­out ever return­ing even a sin­gle number, because 				whether the first number is a 2 or a 1 depends on all of the infi­nite preci­sion in the operands.
\end{itemize}
\end{frame}
