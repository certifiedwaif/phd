...
Take $q$ to be the $N(\mu_{q(\vbeta)}, \sigmaqbeta)$ density:

\[
q(\vbeta; \mu_{q(\vbeta)}, \sigmaqbeta) = (2 \pi)^{-p/2} | \sigmaqbeta |^{-\half}
	\exp{\{ -\half (\vbeta - \mu_{q(\vbeta)})^T \sigmaqbeta^{-1} (\vbeta - \mu_{q(\vbeta)}) \}}
\]
...
\begin{align*}
\log{p(\vy;\vmu_{q(\vbeta)}, \sigmaqbeta)} &= \vy^T X \mu_{q(\vbeta)} -
	1_n^T \exp{\mX \vmu_{q(\vbeta)} + \half \diag{\mX \sigmaqbeta X^T}} \\
&-half (\mu_{q(\vbeta)} - \vmu_{\vbeta})^T \sigmaqbeta^{-1} (\mu_{q(\vbeta)} - \vmu_{\vbeta}) -\half \tr{\Sigma_{\beta}^{-1} \sigmaqbeta}\\
& + \half \log{|\sigmaqbeta|} - \half \log{|\Sigma_{\vbeta}|} + \frac{k + 1}{2}\\
& -1_n^T \log{y!}
\end{align*}
...
\begin{align*}
\frac{\partial \log{p(\vy; \vmuqbeta, \sigmaqbeta)}}{\partial \vmuqbeta} &= y^T X - 1_n^T \exp{\left { X \vmuqbeta + \half \diag{frogs} \right }}
\end{align*}

Partially differentiating with respect to the parameters we obtain

\ldots

and inverting the derivatives allows us to perform Newtown-Raphson updates on the parameters
using the current value of the q-log likelihood.
