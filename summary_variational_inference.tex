...
$p(x_i|a_{i1}, \ldots, a_{ik}) = \Pi_{k=1}^K (2\pi \sigma_k^2)^{-\frac{1}{2}}\exp{(\mu_k- \sigma_{k})^2})^{a_{ik}}$, then the joint distribution is easily shown to be as in
\eqref{eq:gaussian_mixture}. % So show it then.
