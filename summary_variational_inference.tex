...
&= \half \log{2 \pi} - \half \frac{(\mu_k - \mu_{\mu_k})^2}{\sigma_{\mu_{\mu_k}^2}}
\end{align*}
...
\propto&- \half \frac{A_k}{B_k} \\
&+ \psi{\alpha_{q_{w_{ik}}} }\\
&+ (\alpha - 1) \psi{\alpha_{q_{w_{ik}}} \right]\\
& - \frac{(x_i - \mu_k)^2}{\frac{A_k}{B_k}} \right]\\
&+ A_k \log{B_k} + \Psi{A_k} - (A_k - 1) \log{\sigma_k^2} - B_k \frac{1}{\sigma_k^2} \right ]
\end{align*}
