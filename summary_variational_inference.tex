...
%&+ (A_k - 1) \log{\sigma_k^2} - B_k \frac{1}{\sigma_k^2} \right ]
\end{align*}
...
Take $q$ to be the $N(\mu_{q(\vbeta)}, \Sigma_{q(\vbeta)})$ density:

\[
q(\vbeta; \mu_{q(\vbeta)}, \Sigma_{q(\vbeta)}) = (2 \pi)^{-p/2} | \Sigma_{q(\vbeta)} |^{-\half}
	\exp{\{ -\half (\vbeta - \mu_{q(\vbeta)})^T \Sigma_{q(\vbeta)}^{-1} (\vbeta - \mu_{q(\vbeta)}) \}}
\]

Then the lower bound admits the explicit expression
% FIXME: This LaTeX needs to be corrected so that it displays
\begin{align*}
\log{p(\vy;\vmu_{q(\vbeta)}, \Sigma_{q(\vbeta)})} &= \vy^T X \mu_{q(\vbeta)} -
	1_n^T \exp{\mX \vmu_{q(\vbeta)} + \half \diag{\mX \Sigma_{q(\vbeta)} X^T}} \\
&-half (\mu_{q(\vbeta)} - \vmu_{\vbeta})^T \Sigma_{q(\vbeta)}^{-1} (\mu_{q(\vbeta)} - \vmu_{\vbeta}) -\half \tr{\Sigma_{\beta}^{-1} \Sigma_{q(\vbeta)}}\\
& + \half \log{|\Sigma_{q(\vbeta)}}|} - \half \log{|\Sigma_{\vbeta}|} + \frac{k + 1}{2}\\
& -1_n^T \log{y!}
\end{align*}
