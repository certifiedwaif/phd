...
% TODO: This whole section is terse, and the wording and ordering is strange.
...
\end{array}
$$
with $p_i = \expit\left[ \psi{(\alpha_{q(\rho)})} - \psi{(\beta_{q(\rho)})} - \exp{(c_i^\top\vmu + \half c_i^\top \mLambda c_i)} \right]$ when $\vy_i = 0$, and $1$ otherwise.
		...
		\STATE $\eta \leftarrow -\exp \left [ \mC \vmu + \half \diag{(\mC\mLambda\mC^\top)} \right ] + \psi{(\alpha_{q{(\rho)}})} - \psi{(\beta_{q{(\rho)}})}$ \\[1ex]
			...
			\begin{algorithm}
				...
				\begin{algorithmic}
					...
					\ENDWHILE \\
					* Step to ensure numeric stablity
				\end{algorithmic}
			\end{algorithm}
			...
			\begin{align*}
				\log \underline{p}(\vmu, \mLambda; \vy) & = \phantom{-} \vy^\top\mP \mC \vmu - \vp^\top \exp[\mC \vmu + \half \text{diag}(\mC \mLambda \mC^\top)] - \half \vmu^\top \mSigma^{-1} \vmu - \half \tr{(\mSigma^{-1} \mLambda)} + \log{|\mR|} \\
			...
			\begin{figure}[p]
				\caption{Top left: Covariance matrix of a random slope model with ten groups (m=10, b=2) -- Fixed effects before random effects,
									top right: Covariance matrix of the same model -- Random effects before fixed effects,
									bottom left: Cholesky factor of the covariance matrix of the same random slope model -- Fixed effects before random effects,
									bottom right: Cholesky factor of the covariance matrix of the same random slope model -- Random effects before fixed effects. Note the density of the bottom left Cholesky factor, and the
									sparsity of the bottom right Cholesky factor}
				\label{fig:covfixedrandom}
				\includegraphics[scale=.4]{mX_mZ_mLambda.pdf}
				\includegraphics[scale=.4]{mZ_mX_mLambda.pdf}
				\includegraphics[scale=.4]{mX_mZ_cholesky.pdf}
				\includegraphics[scale=.4]{mZ_mX_cholesky.pdf}
			\end{figure}
			...
			\begin{align*}
				\log \underline{p}(\vmu, \mLambda; \vy) & = \phantom{-} \vy\mP\mC \vmu - \vp^\top \exp(\mC \vmu + \half \text{diag}(\mC \mLambda \mC^\top)) - \half \vmu^\top \mSigma^{-1} \vmu - \half \tr{(\mSigma^{-1} \mLambda)} \\
			...
			\begin{align*}
				\frac{\partial \log \underline{p}(\vmu, \mLambda; \vy)}{\partial \vmu}     & = \quad \mC^\top\vp \left [\vy - \mC\exp(\mC \vmu + \half \text{diag}(\mC \mLambda \mC^\top)) \right ] - \mSigma^{-1} \vmu \phantom{-} \text{and} \\
			...
			\begin{algorithm}
				...
				\begin{algorithmic}
					...
					\STATE \text{Calculate} $\mLambda \leftarrow \left (\mLambda^{-1} \right )^{-1}$  using the block inverse formula \\ [1ex]
					...
					\ENDWHILE \\
					* this step was added to improve numeric stability
				\end{algorithmic}
			\end{algorithm}
			...
			$\mLambda^{-1}$, as can be seen in Figure \ref{fig:covfixedrandom}. Thus the Gaussian $q(\vnu) \sim \N(\vmu, \mLambda)$ can be optimised over a space of dimension $\half p (p + 1) + pq + \half q (q + 1)$ rather than dimension
			...
			\[
				\diag(\mC \mLambda \mC^\top)_{ii} = \mC_{i .} \mR \mR^\top \mC_{i .}^\top,\phantom{-} 1 \leq i \leq n
			\]
			...
			\[
				\diag(\mC \mLambda \mC^\top)_{ii} = \mC_{i .} \mR^{-\top} \mR^{-1} \mC_{i .}^\top,\phantom{-} 1 \leq i \leq n,
			\]
			...
			\[
				\diag(\mC \mLambda \mC^\top)_{ii} = \va^\top \va,\phantom{-} 1 \leq i \leq n,
			\]
			...
			\begin{table}
				...
				\begin{tabular}{|l|rr|}
					...
					GVA $\mLambda = \mR^\top \mR$        & 2.04         & 1.24                       \\
					% Why is this slower?
					GVA $\mLambda = (\mR^\top \mR)^{-1}$ & 0.38         & 0.66                       \\
					GVA NR FP                            & 0.05         & 0.07                       \\
					\hline
				\end{tabular}
			\end{table}
